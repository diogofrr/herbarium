...
export function useGeolocation(dispatch: Dispatch<MapAction>) {
  return useCallback(() => {
    if (!navigator.geolocation) {
      dispatch({ type: "SET_ERROR", payload: "Geolocalização não suportada neste navegador." });
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        ...
        dispatch({ type: "SET_FOCUS", payload: { ...pt, zoom: 16, nonce: Date.now() } });
        dispatch({ type: "SET_LOCATING", payload: false });
      },
      () => {
        dispatch({ type: "SET_ERROR", payload: "Não foi possível obter sua localização." });
        dispatch({ type: "SET_LOCATING", payload: false });
      },
